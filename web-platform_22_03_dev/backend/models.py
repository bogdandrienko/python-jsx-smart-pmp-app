...
class ModulesModel(models.Model):
    """
    Модель, которая содержит объекты, которые отображаются как модули, секции или компоненты веб-платформы
    """
...
class NotificationModel(models.Model):
    """
    Модель, которая содержит уведомления для определённой группы пользователей веб-платформы
    """
...
class OrderModel(models.Model):
    user = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True)
    paymentMethod = models.CharField(max_length=254, null=True, blank=True)
...
class ShippingAddressModel(models.Model):
    order = models.OneToOneField(OrderModel, on_delete=models.CASCADE, null=True, blank=True)
    address = models.CharField(max_length=254, null=True, blank=True)
    city = models.CharField(max_length=254, null=True, blank=True)
    postalCode = models.CharField(max_length=254, null=True, blank=True)

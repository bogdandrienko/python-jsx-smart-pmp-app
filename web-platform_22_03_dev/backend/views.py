...
from backend import models as backend_models, settings as backend_settings, serializers as backend_serializers, \
    forms as backend_forms, service as backend_service
...
def api_rational(request):
    ...
    try:
        ...
        #######################################################################
            ...
            if request_action_type == "RATIONAL_CREATE":
                try:
                    ...
                    additional_word = request.data.get("additionalWord")
                    if not additional_word:
                        additional_word = None
                    additional_pdf = request.data.get("additionalPdf")
                ...
                try:
                    ...
                    backend_service.DjangoClass.LoggingClass.logging_errors(request=request, error=error)
                    return Response({"error": "Произошла ошибка!"})
            elif request_action_type == "RATIONAL_LIST":
                try:
                    sphere = request.data.get("sphere")
                    subdivision = request.data.get("subdivision")
...
def api_user_all_(request):
    ...
    request_method, request_action_type, request_user, request_body = \
        backend_service.DjangoClass.DRFClass.request_utils(request=request)
    # print(f"datetime: {backend_service.DateTimeUtils.get_current_datetime()} | ", f"method: {request_method} | ",
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if password and email_:
                subject = 'Восстановление пароля от веб платформы'
                encrypt_message = backend_service.EncryptingClass.encrypt_text(
...
def create_modules(request):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if actions:
                workbook = backend_service.ExcelClass.workbook_load(excel_file=actions)
                ...
                for row in range(2, backend_service.ExcelClass.get_max_num_rows(sheet) + 1):
                    try:
            ...
            if groups:
                ...
                for row in range(2, backend_service.ExcelClass.get_max_num_rows(sheet) + 1):
                    try:
                        name_char_field = backend_service.ExcelClass.get_sheet_value('A', row, sheet)
                        name_slug_field = backend_service.ExcelClass.get_sheet_value('B', row, sheet)
                        action_many_to_many_field = backend_service.ExcelClass.get_sheet_value('D', row, sheet)
                        user_many_to_many_field = backend_service.ExcelClass.get_sheet_value('E', row, sheet)
                        position_float_field = backend_service.ExcelClass.get_sheet_value('C', row, sheet)
                ...
                backend_service.ExcelClass.workbook_close(workbook=workbook)
            if _modules:
                workbook = backend_service.ExcelClass.workbook_load(excel_file=_modules)
                sheet = backend_service.ExcelClass.workbook_activate(workbook=workbook)
                ...
                for row in range(2, backend_service.ExcelClass.get_max_num_rows(sheet) + 1):
                    try:
                        module = backend_models.ModulesModel.objects.get_or_create(
                            next_path_slug_field=backend_service.ExcelClass.get_sheet_value('E', row, sheet)
                        )[0]
                        module.type_slug_field = dictionary[
                            str(backend_service.ExcelClass.get_sheet_value('A', row, sheet)).strip()]
                        module.name_char_field = backend_service.ExcelClass.get_sheet_value('B', row, sheet)
                        module.previous_path_slug_field = backend_service.ExcelClass.get_sheet_value('C', row, sheet)
                        module.current_path_slug_field = backend_service.ExcelClass.get_sheet_value('D', row, sheet)
                        module.position_float_field = backend_service.ExcelClass.get_sheet_value('F', row, sheet)
                        module.text_field = backend_service.ExcelClass.get_sheet_value('G', row, sheet)
                        module.save()
                    except Exception as error:
                        backend_service.DjangoClass.LoggingClass.logging_errors(request=request, error=error)
                backend_service.ExcelClass.workbook_close(workbook=workbook)
    ...
    if True:
        if request.method == 'POST':
            ...
            if type_action == 'change':
                ...
                notification.decision_datetime_field = backend_service.DateTimeUtils.get_current_datetime()
                notification.save()
...
def account_create_or_change_accounts(request):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if excel_file:
                try:
                    workbook = backend_service.ExcelClass.workbook_load(excel_file=excel_file)
                    sheet = backend_service.ExcelClass.workbook_activate(workbook=workbook)
                    for row in range(2, backend_service.ExcelClass.get_max_num_rows(sheet) + 1):
                        try:
                            # authorization data
                ...
                for user_object in user_objects:
                    try:
                        ...
                        if groups:
                            ...
                            groups = ''
                        backend_service.ExcelClass.set_sheet_value('O', _index, groups, sheet)

                        _email = user_model.email_field
                        backend_service.ExcelClass.set_sheet_value('P', _index, _email, sheet)
...
def salary_(request):
    ...
    try:
        date = [int(x) for x in str(backend_service.DateTimeUtils.get_current_date()).split('-')]
        year = date[0]
        month = date[1] - 1
        day = date[2]
        if day < 10:
...
def analyse(request):
    ...
    h = httplib2.Http(backend_service.DirPathFolderPathClass.create_folder_in_this_dir(folder_name='static/media/temp'))
...
def passages_insert(request):
    ...
    if request.method == 'POST':
        ...
        try:
            connect_db = backend_service.SQLClass.pyodbc_connect(

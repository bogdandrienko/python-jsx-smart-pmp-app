...
class MyWidget(QtWidgets.QWidget):
    def __init__(self, title="app", width=640, height=480, icon=""):
        ...
        self.window_width = QtWidgets.QTextEdit(f"width : 640")
        self.horizontal_box_renderer.addWidget(self.window_width)

        # Height window renderer
        self.window_height = QtWidgets.QTextEdit(f"height : 480")
        self.horizontal_box_renderer.addWidget(self.window_height)
    ...
    def play_btn_func(self):
        play_analiz(ip_entry=str(self.combo.currentText().strip()),
                    ...
                    width=int(*re.findall(r'\d+', self.window_width.toPlainText().strip())),
                    height=int(*re.findall(r'\d+', self.window_height.toPlainText().strip())))

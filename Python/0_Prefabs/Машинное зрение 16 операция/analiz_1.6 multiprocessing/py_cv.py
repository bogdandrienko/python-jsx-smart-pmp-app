...
class AnalyzeClass:
    ...
    def start_analyze(data: dict):
        sources = AnalyzeClass.get_sources(source_type=data['source_type'], sources=data['ip_cam'],
        ...
        try:
            ...
            if data['render_debug'] == 'all':
                ...
                AnalyzeClass.render_threshold(image=image, name=name,
                                              resolution_debug=data['resolution_debug'])
                AnalyzeClass.render_inrange(image=image, sensetivity_analysis=data['sensetivity_analysis'],
                                            name=name,
                                            resolution_debug=data['resolution_debug'])
    ...
    def make_snapshot(data: dict):
        ...
        sources = f"{data['protocol_cam_type']}://192.168.{data['ip_cam']}:{data['port_cam']}/" \
                  f"ISAPI/Streaming/channels/101/picture?snapShotImageType=JPEG"
        response, content = h.request(sources)
        with open(f"{data['ip_cam']}.jpg", 'wb') as file:
            file.write(content)

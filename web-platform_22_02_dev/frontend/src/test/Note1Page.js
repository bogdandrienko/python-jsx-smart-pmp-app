...
import { detailNotes } from "./noteActions";
...
function Note1Page() {
  ...
  let deleteNote = async () => {
    ...
    await axios.delete(`/api/note_api/${id}/`, config);
    navigate("/notes");
  };
  ...
  return (
    <div>
      ...
      <Row>
        {loading ? (<LoaderComponent />) : error ? (<MessageComponent variant="danger">{error}</MessageComponent>) : !notes ? (<MessageComponent variant="danger">{"Unexpected error"}</MessageComponent>) :
        (
